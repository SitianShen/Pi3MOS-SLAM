...
def preprocess_tensor_for_pi3(tensor, PIXEL_LIMIT=255000):
    """
    Preprocess a PyTorch tensor image to the size expected by PI3 model.
    
    Args:
        tensor (torch.Tensor): Input image tensor with shape [C, H, W] or [H, W, C]
        PIXEL_LIMIT (int): Maximum number of pixels (default: 255000)
    
    Returns:
        torch.Tensor: Preprocessed tensor with shape [C, H, W] where H and W are multiples of 14
    """
    import math
    from torchvision import transforms
    ...
    if tensor.dim() == 3:
        if tensor.shape[0] == 3:  # Already [C, H, W]
            pass
        elif tensor.shape[2] == 3:  # [H, W, C] -> [C, H, W]
    ...
    # Calculate target size following the same logic as load_images_as_tensor
    scale = math.sqrt(PIXEL_LIMIT / (W_orig * H_orig)) if W_orig * H_orig > 0 else 1
    W_target, H_target = W_orig * scale, H_orig * scale
